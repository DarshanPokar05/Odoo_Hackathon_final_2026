...
// Lazy-require to avoid circular dependency (invoices → subscriptions → quotations)
function getInvoiceSvc() {
  return require('../invoices/service');
}
...
async function submit(id, user) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (requiredLevel === 'NONE') {
      ...
      // Create the Order record for fulfillment + billing
  ...
  // If auto-confirmed, create invoice + subscriptions
  if (result.newStatus === 'CONFIRMED') {
    try {
      const order = await prisma.order.findUnique({ where: { quotationId: id } });
      if (order) {
        await getInvoiceSvc().processOrderConfirmation(order.id, user);
      }
    } catch (err) {
      // Invoice/subscription creation errors must not roll back the confirmation
      console.error('[quotations/submit] processOrderConfirmation failed:', err.message);
    }
  }
