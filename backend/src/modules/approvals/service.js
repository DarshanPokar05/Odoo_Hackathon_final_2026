...
async function approve(stepId, body, user) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (isFinal) {
      ...
      // Create the Order record so fulfillment + billing can run
  ...
  if (result.isFinal) {
    // Create invoice + subscriptions for the confirmed order
    try {
      const order = await prisma.order.findUnique({ where: { quotationId: step.quotationId } });
      if (order) {
        await getInvoiceSvc().processOrderConfirmation(order.id, user);
      }
    } catch (err) {
      console.error('[approvals/approve] processOrderConfirmation failed:', err.message);
    }
